...
public abstract class Provider {
	...
	protected String getEnv(final String provider, final String var) {
		final String envKey = ENV_PREFIX + getEnvProvider(provider.toUpperCase()) + var;
		final String envValue = System.getenv(envKey);
		if (envValue != null && !envValue.isBlank()) {
			return envValue;
		}
		if (environment != null) {
			final String propertyKey = toPropertyKey(provider, var);
	...
	protected String toPropertyKey(final String provider, final String var) {
		return NAME_PREFIX + provider + "." + var.substring(1).toLowerCase().replace('_', '-');
	}
